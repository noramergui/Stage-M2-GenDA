...
# ── Région à entraîner — changer ici selon l'expérience ──────────────────
region = 'azores' 
# region = 'gulfstream'     

# ── Chemins ───────────────────────────────────────────────────────────────
data_dir   = f'/data2/nora/GenDA_workspace/input_data_{region}/'
output_dir          = f'/data2/nora/GenDA_workspace/experiments/exp_{region}_200kimg_128px_v2/'
...
logger  = PythonLogger(name='train_glorys')
...
dataset_train = Diffusion_Training_Dataset(
    data_dir   = data_dir,
    n_lon      = 128,
    n_lat      = 128,
    variables = ['zos'],
    date_range = [date(2010,1,1), date(2010,9,30)],
...
dataset_val = Diffusion_Training_Dataset(
    data_dir   = data_dir,
    n_lon      = 128,
    n_lat      = 128,
    variables = ['zos'],
    date_range = [date(2010,10,1), date(2010,11,30)],
    var_stds   = rescale_factors,
    lon_buffers= [12, 12],
    lat_buffers= [12, 18],
    multiprocessing = False,
...
# ── Hyperparamètres ────────────────────────────────────────────────────────
c.total_kimg        = 200        # mini-test : 10 kimg (~2500 pas avec batch=4)  # durée totale de l'entraînement en kimg
                                # vrai entraînement : 200                       
c.ema_halflife_kimg = 0.5         # mini-test : 1 kimg                            # demi vie de l'EMA (Exponential Moving Average). Copie lissée des poids du réseau utilisée à l'inférence. Concrètement : après ema_halflife_kimg kimg d'entraînement, l'influence d'un ancien poids sur l'EMA a été divisée par 2. Plus la valeur est grande, plus l'EMA est lisse mais réagit lentement aux nouvelles mises à jour. Pour GenDA : 500 kimg.
                                # vrai entraînement : 500
c.batch_size_global = 64         # mini-test                                     # taille du batch
                                # vrai entraînement : 64
c.batch_size_gpu    = 64         # Nombre de patches tratés par GPU à chaque pas. Si on a 2 GPUs et batch size global = 64, alors batch size gpu = 32
...
c.state_dump_ticks  = 10         # sauvegarde tous les 5 ticks

c.valid_dump_ticks  = 10         # fréquence de calcul de la loss de validation  

c.num_validation_evals = 10      # nombre de batches pour estimer la loss de validation

c.lr_rampup_kimg = 10            # durée du warmup du learning rate en kimg. Exemple lr_rampup_kimg=10000 : le warump dure 10kimg sur 200, soit 5% de l'entrainement, c'est raisonnable.
                                # vrai entraînement : 10000
c.run_dir           = output_dir

c.wandb_mode    = 'online'

c.wandb_project = 'GenDA-GLORYS'

c.wandb_entity  = 'nora-mergui-universit-grenoble-alpes'   # remplace par ton username

c.wandb_name    = f'exp_{region}_200kimg_v2'

c.wandb_group   = 'GLORYS'

logger0.info('Démarrage entraînement GenDA/glorys')
