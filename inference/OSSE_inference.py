...
for t in range(365):
    ...
    total_mask = np.stack((ds_masks['ssh_total'].isel(time = t),  # canal 0 : SSH — trace altimètre (sparse)
                           ds_masks['sst_mask'].isel(time = t), # canal 1 : SST — masque nuages
                           np.zeros((128,128)), # canal 2 : SSS — non observé directement
                           np.zeros((128,128)), # canal 3 : u   — non observé
                           np.zeros((128,128)), # canal 4 : v   — non observé
                           np.ones((128,128)), # canal 5 : uas — vents ERA5 partout
                           np.ones((128,128)), # canal 6 : vas — vents ERA5 partout
                          ), axis = 0)
    ...
    def A(x):
        # 1. Observations instantanées éparses (L3) : SSH sur la trace altimètre + SST
        inst_obs = x[:, total_mask.astype('bool')]

        # 2. Remettre à l'échelle physique
        ...
        # 3. Lissage gaussien spatialement différencié par variable
        ...
        # 4. Re-normaliser et masquer (ne garder que là où l'OI existe)
        smoothed_obs = smoothed_obs[:,oi_mask]
        
        # 5. Concaténer obs éparses + obs lissées
