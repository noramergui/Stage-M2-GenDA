...
# 1. le fichier
ds_oi = xr.open_dataset(data_dir + 'OI.nc')
ds_oi = (ds_oi[['gssh']]
...
ds_oi = ds_oi.sel(longitude=LON_GRID, latitude=LAT_GRID,
...
# ── Paramètres bruit (SSH) ───────────────────────────────────────
nad_noise_phys = 0.05      # 5 cm (mètres)
swot_noise_phys = 0.03
#oi_err_phys    = 0.10    # 10 cm (mètres)
nad_noise_std  = nad_noise_phys / rescale    # normalisé
swot_noise_std = swot_noise_phys / rescale
#oi_err_std     = oi_err_phys / rescale       # normalisé

from scipy.ndimage import gaussian_filter

print('estimating oi_err_std...')
_err = []
for _d in ds_oi.time.values:
    _t = int(np.where(dataset.ds_model['time'].values == _d)[0][0])
    _truth = dataset.__getitem__(_t)[0].numpy()
    _truth_smooth = gaussian_filter(_truth, sigma=[sigma_lat_ssh, sigma_lon_ssh])
    _oi = ds_oi['ssh'].sel(time=_d).values
    _err.append(_oi - _truth_smooth)              # <-- résidu APRÈS lissage
_err = np.stack(_err)[:, 8:-8, 8:-8]
oi_err_std = float(np.nanstd(_err))
print(f'oi_err_std = {oi_err_std:.4f} (norm) | {oi_err_std * rescale:.4f} m')

for s in [30, 40, 50, 62, 75, 90, 110]:
    sl = (1/NN_res) * s / deg_lat_in_km
    slo = (1/NN_res) * s / deg_lon_in_km
    e = [ds_oi['ssh'].sel(time=_d).values
         - gaussian_filter(dataset.__getitem__(int(np.where(dataset.ds_model['time'].values == _d)[0][0]))[0].numpy(), sigma=[sl, slo])
         for _d in ds_oi.time.values]
    print(s, float(np.nanstd(np.stack(e)[:, 8:-8, 8:-8])))
...
np.random.seed(0) # pour que les 3 runs voient eactement le même bruit d'obs
...
for day_str in TEST_DATES:
    ...
    # champ de niveau de bruit : 0.05 sur nadir, 0.03 sur swot
    noise_level_map = np.zeros((128, 128), dtype='float32')
    noise_level_map[m_nadir] = nad_noise_std
    noise_level_map[m_swot]  = swot_noise_std   # si chevauchement, swot (moins bruité) l'emporte

    total_mask = m_all[None]                     # (1,128,128)
    noise = torch.from_numpy(np.random.randn(128,128).astype('float32') * noise_level_map)[None]
    noise_levels = noise_level_map[None]         # (1,128,128)
    ...
    # on remplace l'OI par le résultat de l'assimilation de la veille 
    #from scipy.ndimage import gaussian_filter

    #x_prev = np.load(pred_dir + 'pred2017-06-12.npy')
    #prev_mean = x_prev[:, 0].mean(0)                          # (128,128) normalisé

    #print(f'  prev_mean : range [{prev_mean.min():.2f}, {prev_mean.max():.2f}] | std {prev_mean.std():.2f}')

    # lisser comme le fait A (même sigma en pixels)
    #prev_mean_smooth = gaussian_filter(prev_mean, sigma=[sigma_lat_ssh, sigma_lon_ssh])

    #print('après lissage : range', round(prev_mean_smooth.min(),2), round(prev_mean_smooth.max(),2),
    #  '| std', round(prev_mean_smooth.std(),2))
    
    #oi_field = prev_mean_smooth

    # masque grande-échelle : tout le domaine sauf bords 8px
    #ls_mask = np.ones((128,128), dtype=bool)
    #ls_mask[:8,:] = False; ls_mask[-8:,:] = False
    #ls_mask[:,:8] = False; ls_mask[:,-8:] = False
    #oi_mask = ls_mask[None]
    #oi_ground_truth = torch.from_numpy(oi_field[None, None].astype('float32'))
    ...
    nom_exp = f'pred{day_str}_oi.npy'

    np.save(pred_dir + nom_exp, x)
    ...
    print(f'  sauvegarde de {nom_exp}.png ...')
    plt.savefig(pred_dir + f'{nom_exp}.png', dpi=110, bbox_inches='tight')
